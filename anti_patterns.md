...
### AP-027: Colon + Space in Workflow Description (Antigravity Bug)
- **現象**: ワークフローの `description`（または `Description`）フィールドにおいて、「：」（コロン）の直後に半角スペースが続くと、そのワークフローがスラッシュコマンド一覧から消失する。
- **原因**: Antigravity内部のパーサーまたはUIコンポーネントにおける特定の文字列パターンの誤認。
- **防止策**:
  - `description` 内でコロンを使用する場合は、直後にスペースを置かない、または読点（、）などで代用する。
  - 基本的に `description` は簡潔な一文とし、記号の使用を避ける。
...
| 2026-03-12 | AP-027 | Agent | ワークフロー表示消失バグ（コロン＋スペース）と解決策（コロン直後のスペース回避） |
| 2026-02-26 | AP-020022 | Agent | Lingerfindwordpress/stock2 |
| 2026-03-08 | AP-023 | Agent | 自壊事故（自己死滅）の教訓 |

---

### AP-027: ワークフロー description 内のコロン＋スペース問題
- **カテゴリ**: ワークフロー / UI整合性
- **現象**: ワークフローのフロントマター `description` フィールドにおいて、`プロパティ名: 値` のようにコロンの直後にスペースを置くと、そのワークフローが Antigravity のコマンド一覧から非表示になる。
- **原因**: 内部パーサーがフロントマターをネストされたキーと誤認し、最上位の `description` 属性として認識できなくなるため。
- **解決策**:
  - コロンを使用しない。
  - コロンを使用する場合、直後にスペースを入れない（例：`Step 1:Deepen`）。
  - または全角コロン（：）を使用する。
