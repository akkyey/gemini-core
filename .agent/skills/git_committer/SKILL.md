...
3.  **マルチリポジトリのステージング (Multi-repo Staging)**:
    - サブモジュールを含む構成の場合、親リポジトリのルートで診断を実行する前に、**親とサブモジュールの両方で変更をステージング** (`git add .`) してください。
    - これにより、全域にわたる整合性チェック（`full_context` の同期等）がパス可能な状態になります。

4.  **決定的な診断実行 (Deterministic Diagnostics)**:
    - 診断プログラム（`self_diagnostic.py` 等）の差分チェックにおいて、タイムスタンプなどの非決定的な要素が原因で失敗する場合は、`git diff -I "^Generated at:"` のように不変部分のみを比較対象とするようにプログラムまたはコマンドを調整してください。
