...
### Phase 0: Anti-Pattern Sync (アンチパターン同期)
1.  **最新化**:
    ```bash
    cd .gemini && git pull origin main && cd ..
    ```
2.  `.gemini/anti_patterns.md` を参照し、既知のアンチパターンを把握する。
...
2.  **アンチパターン特定**: 「これはライブラリで代替できないか？単純化できないか？」を検討する。
    *   `anti_patterns.md` に該当するパターンがないか確認する。
...
### Phase 5: Anti-Pattern Update (アンチパターン蓄積)
1.  **新規発見時**:
    *   今回のリファクタリングで発見したアンチパターンが `anti_patterns.md` に未記載の場合:
        1. `anti_patterns.md` に追記
        2. `.gemini` サブモジュールをコミット & プッシュ
2.  **コミットメッセージ**:
    *   `refactor(anti-pattern):` プレフィックスを使用
