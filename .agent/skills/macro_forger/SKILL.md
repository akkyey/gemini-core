...
1.  **コマンドと引数**: `./venv/bin/python` 等の正確なバイナリパスを確認。**物理隔離環境（Tier 3）での動作を保証するため、常に絶対パスを指定する。**
2.  **環境変数**: `PYTHONPATH` やアプリケーション固有の `ENV` を定義。
3.  **セマンティクス制御**: `PortBindings` や `Env`（DB接続情報等）、`NetworkMode` 等のシステム構成要素を引数や環境変数として設計し、AI が詳細なフラグ操作ではなく「構成の意味（Semantics）」を制御できるようにする。
4.  **テンプレート化**: 引数の可変部分を `${args}` や `${1}` として設計。
